...
class AiGenerationService
{
    ...
    private function startMagicEraser(array $payload, string $shopDomain): array
    {
        ...
        if (isset($advancedConfig['num_inference_steps']) && is_numeric($advancedConfig['num_inference_steps'])) {
            $apiPayload['input']['num_inference_steps'] = (int) $advancedConfig['num_inference_steps'];
        }
    ...
    /**
     * Merge file config with SiteSetting overrides and normalize booleans.
     */
    ...
    private function postReplicatePredictionWithRetry(string $token, array $payload, string $logChannel, array $context = [])
    {
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            try {
                ...
                Log::channel($logChannel)->warning('Replicate transient error, retrying', array_merge($context, [
                    'attempt' => $attempt,
                    'status' => $status,
                ...
                $delaySeconds = $backoffStrategy === 'linear' ? $attempt : (2 ** ($attempt - 1));
                Log::channel($logChannel)->warning('Replicate call exception, retrying', array_merge($context, [
                    'attempt' => $attempt,
                    'delay_seconds' => $delaySeconds,
                    'error' => $e->getMessage(),
                ]));
    ...
    private function startEnhanceJob(array $payload, string $shopDomain): array
    {
        ...
        $settings = \App\Models\SiteSetting::getEnhancerSettings();
