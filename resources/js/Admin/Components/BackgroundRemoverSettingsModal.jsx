...
import { X, Save, RotateCcw, Loader, Trash2 } from 'lucide-react';
import axios from 'axios';

// Supported resolutions for BiRefNet
const RESOLUTIONS = [
  { label: 'Auto', value: '' },
...
export default function BackgroundRemoverSettingsModal({ isOpen, onClose, onSave }) {
  ...
  const loadSettings = async (showRefresh = false) => {
    try {
      ...
      const response = await axios.get('/admin/background-remover-settings?format=minimal');
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      setError(null);
      await axios.put('/admin/background-remover-settings', settings);
  ...
  const handleReset = async () => {
    ...
    try {
      setSaving(true);
      setError(null);
      const response = await axios.post('/admin/background-remover-settings/reset');
      const newSettings = response.data.settings;
      cacheRef.current = newSettings;
      setSettings(newSettings);
      setOriginalSettings(newSettings);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-2xl my-8">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Background Remover Settings</h2>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
                  <Trash2 size={16} className="text-primary-600" />
                  Model Version
                </label>
                <input
                  type="text"
                  ...
                  placeholder="e.g., men1scus/birefnet"
                ...
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Specify the BiRefNet model version for background removal</p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g., 'clean background removal, keep subject edges sharp, 8K quality'"
                  rows={3}
                ...
                <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                  Instructions to improve background removal quality and consistency.
                </p>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        Format: WxH (e.g., 1024x1024) or leave empty for auto resolution
                      </p>
